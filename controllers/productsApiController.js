...
const getProducts = async (req,res) => {
    if (req.params.id) { // con ID
        ...
        try {
            let products = await Product.find({},'-_id -__v').populate('provider', 'company_name address'); 
...
const createProduct = async (req, res) => {
    ...
    try {
        const foundProvider = await Provider.findOne({ company_name: newProduct.provider });

        if (!foundProvider) {
            return res.status(400).json({ message: "Proveedor no encontrado" });
        }

        newProduct.provider = foundProvider._id; // Asignar el ID del proveedor al campo 'provider'

        const response = await Product.create(newProduct); // Crear y guardar el nuevo producto en la base de datos
